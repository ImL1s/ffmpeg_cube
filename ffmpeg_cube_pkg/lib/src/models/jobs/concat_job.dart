...
class ConcatJob extends BaseJob {
  ...
  List<String> toFFmpegArgs() {
    ...
    if (method == ConcatMethod.demuxer) {
      ...
      final filterInputs = List.generate(inputPaths.length, (i) => '[$i:v][$i:a]').join('');
      final filterComplex = '${filterInputs}concat=n=${inputPaths.length}:v=1:a=1[outv][outa]';
