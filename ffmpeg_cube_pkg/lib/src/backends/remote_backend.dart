...
class RemoteBackend implements FFmpegBackend {
  ...
  Future<JobResult<void>> execute(
    ...
    try {
      ...
      if (inputIndex == -1 || inputIndex + 1 >= args.length) {
        return JobResult.failure(JobError.validation('No input file specified'));
      }
      ...
      final request = http.MultipartRequest('POST', Uri.parse('$endpoint/process'));
      request.headers.addAll(_headers);
      request.files.add(await http.MultipartFile.fromPath('file', inputPath));
      request.fields['args'] = jsonEncode(args);
      
      onProgress?.call(JobProgress(progress: 0.0, totalDuration: totalDuration));
  ...
  Future<JobResult<ProbeResult>> probe(String filePath) async {
    ...
    try {
      ...
      final request = http.MultipartRequest('POST', Uri.parse('$endpoint/probe'));
      request.headers.addAll(_headers);
